...
def wait_for_config(manager, max_wait_time, sleep_duration):
    ...
    click.echo("Config backend is not ready after {} seconds.".format(max_wait_time))
    sys.exit(1)
...
def wait_for_secret(manager, max_wait_time, sleep_duration):
    for i in range(0, max_wait_time, sleep_duration):
        try:
            ...
            click.echo("Secret backend is ready.")
            return
        except Exception as exc:
            click.echo(
    ...
    click.echo("Secret backend is not ready after {} seconds.".format(max_wait_time))
    sys.exit(1)
